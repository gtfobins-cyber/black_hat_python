...
def udp_sender(SUBNET, MAGIC_MSG):
    ...
    for ip in IPNetwork(SUBNET):
        try:
            sender.sendto(bytes(MAGIC_MSG, 'utf-8'), (str(ip), 65535))
        except:
            pass    
...
try:
    while True:
        ...
        if ip_header.protocol == "ICMP":
            ...
            if ping_header.type == 3 and ping_header.code == 3:
                if IPAddress(ip_header.src_address) in IPNetwork(SUBNET):
                    if (raw_buffer[len(raw_buffer) - len(MAGIC_MSG):] == bytes(MAGIC_MSG, 'utf-8')):
                        print(f"The host is up: {ip_header.src_address}")
